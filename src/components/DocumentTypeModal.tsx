...
interface DocumentTypeModalProps {
  ...
  onSave: (
    data: CreateDocumentTypeData | UpdateDocumentTypeData
  ) => Promise<void>;
  onClose: () => void;
...
function DocumentTypeModal({
  ...
  useEffect(() => {
    if (documentType) {
      setFormData({
        ...
        active: documentType.active ? 1 : 0,
      });
      setIsActive(documentType.active);
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (documentType) {
        // Update existing document type (exclude id from update data)
        const { id, ...updateData } = formData;
        await onSave({
          doc_typ_name: updateData.name,
          doc_typ_active: updateData.active,
        });
      } else {
        const { id, ...createData } = formData;
        await onSave({
          doc_typ_name: createData.name,
          doc_typ_active: createData.active,
        } as CreateDocumentTypeData);
      }
    } catch (err) {
  ...
  const handleDelete = async () => {
    if (!documentType || !onDelete) return;

    if (
      !window.confirm(`Are you sure you want to delete "${documentType.name}"?`)
    ) {
      return;
    }
    ...
    setLoading(true);
    try {
      await onDelete(documentType.id);
      onClose();
    } catch (err) {
      setError(
